...
let links = document.querySelectorAll(".header_menu_item");
...
document.getElementById("contact_button").onclick = function () {
    ...
    [ name, tel].forEach(item =>{
        if (!item.value) {
            item.style.border = "thick solid #ED4C5C";
            hasError = true;
        } else {
            item.style.border = "";
        }
    ...
    if (!hasError) {
        ...
        alert("Ваше сообщение принято! Мы скоро свяжемся с вами!");
    }
}
